...
dataset_to_testsize["penn_treebank"]="100 200 500 1000 2000"
dataset_to_testsize["penn_treebank.500"]="100"
dataset_to_testsize["penn_treebank.1000"]="200"
dataset_to_testsize["penn_treebank.1500"]="300"
dataset_to_testsize["penn_treebank.2000"]="1950"
dataset_to_testsize["penn_treebank.2500"]="500"

# method:models
declare -A method_to_model
method_to_model["hsic"]=$(printf "%s:emb64.mlp256x128x32-mlp64x32x8;" "${!dataset_to_testsize[@]}")
    ...
    case $method in
        ...
        c2st-s | c2st-l)
            echo "\
                ...
                --save-dir $save_root/penn_treebank/$dataset/c2st/$model/$run \
                --n-epochs 1000"
            ;;
        *)
            echo "\
                ...
                --save-dir $save_root/penn_treebank/$dataset/$method/$model/$run \
                --n-epochs 1000"
            ;;
    esac
}
    ...
    case $method in
        bandwidth)
            echo "\
                ...
                --data-config $data_root/penn_treebank/$dataset.yml \
                --model-config $model_root/hsic/$model.yml \
                --pretrained-path $save_root/penn_treebank/$dataset/hsic/$model/$run/best.pt \
        ...
        c2st-s)
            echo "\
                ...
                --data-config $data_root/penn_treebank/$dataset.yml \
                --model-config $model_root/c2st/$model.yml \
                --pretrained-path $save_root/penn_treebank/$dataset/c2st/$model/$run/best.pt \
                --log-dir $log_root/penn_treebank/$run \
                --n-samples $n_samples"
            ;;
        c2st-l)
...
# TODO: PLAY WITH THE SPLIT RATIOS UNTIL WE FIND A GOOD ONE 


datasets="penn_treebank.2000"
source train.sh $run "c2st" "$datasets"
source eval.sh $run "c2st-l" "$datasets"
