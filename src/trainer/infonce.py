...
class InfoNCETrainer(BaseTrainer):
    ...
    def inference(self,
        ...
        for i in (pbar:=tqdm(range(n_tests),
            ...
            X = batch[0].to(self.device)    # (N, *, Dx)
            Y = batch[1].to(self.device)    # (N, *, Dy)
            nce, p_value = metrics.infonce.permutation_test(self.model,
                                                            X, Y,
                                                            n_permutations=n_permutations)
            samples.append((nce, p_value))
            pbar.set_description(f"[{i+1}/{n_tests}] infoNCE: {nce}, p-value: {p_value:.4f}")
        return samples
    ...
    def compute_metrics(self,
        ...
        nces, p_values = zip(*samples)
        nces = np.array(nces)
        p_values = np.array(p_values)
        stats = dict()
        stats['infoNCE'] = nces.mean()
