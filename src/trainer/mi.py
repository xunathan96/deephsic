...
class MITrainer(BaseTrainer):
    ...
    def inference(self,
        ...
        for i in (pbar:=tqdm(range(n_tests),
            ...
            X = batch[0].to(self.device)    # (N, *, Dx)
            Y = batch[1].to(self.device)    # (N, *, Dy)
            Tstat, p_value = metrics.mi.permutation_test(self.model,
                                                         X, Y,
                                                         n_permutations=n_permutations)
            samples.append((Tstat, p_value))
            pbar.set_description(f"[{i+1}/{n_tests}] T: {Tstat}, p-value: {p_value:.4f}")
        return samples
    ...
    def type1_error(self,
        ...
        for i in (pbar:=tqdm(range(n_tests),
            ...
            if p_value < significance:
                n_reject += 1
            pbar.set_description(f"[{i+1}/{n_tests}] Tstat: {Tstat}, p-value: {p_value:.4f}")
    ...
    def test_asymptotic_power(self, n_tests: int = 100,):
        ...
        for i in (pbar:=tqdm(range(1),
            ...
            pbar.set_description(f"[{i+1}/{n_tests}] fxy: ") #{fxy:.4f}")

        import pickle
        # with open('snr_mi.pkl', 'wb') as file: 
        #     pickle.dump(snrs, file) 
        with open('gram_mut.pkl', 'wb') as file: 
            pickle.dump(Fxy, file) 
        return 
        with open('T_mi.pkl', 'wb') as file: 
            pickle.dump({
                'T1': T1s,
                'T0': T0s,
                'var_alt': var_alts,
